...
function runAlertCheck(db, flights) {
  ...
  // Pre-fetch all recent dedup keys in one query instead of N×M individual lookups
  let recentSet;
  try {
    const rows = db.prepare(
      `SELECT rule_id || ':' || icao AS key FROM alert_events WHERE triggered_at >= datetime('now', '-30 minutes')`
    ).all();
    recentSet = new Set(rows.map(r => r.key));
  } catch { recentSet = new Set(); }
  ...
  const run = db.transaction(() => {
    for (const rule of rules) {
      for (const flight of flights) {
        ...
        const dedupKey = `${rule.id}:${flight.icao}`;
        ...
        recentSet.add(dedupKey); // prevent duplicates within same batch
        newEvents++;
      }
    }
  });
  ...
  if (newEvents > 0) console.log(`[alerts] ${newEvents} nowych zdarzeń`);
  return newEvents;
}
